...
GLboolean   GLHelper::keystateP;
GLboolean   GLHelper::keystateV;
...
void GLHelper::key_cb(GLFWwindow *pwin, int key, int scancode, int action, int mod) 
{
    switch (action)
    {
        case GLFW_PRESS:
        {
            ...
            // P key for changing polygon mode
            keystateP = (key == GLFW_KEY_P);
            // V key for changing camera mode
            keystateV = (key == GLFW_KEY_V);
