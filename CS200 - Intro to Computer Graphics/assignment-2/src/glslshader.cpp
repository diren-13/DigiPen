...
\date           June 13, 2016
\brief          This file contains definitions of member functions of class GLShader.
...
{
    if (pgm_handle > 0) { glDeleteProgram(pgm_handle); }
}
...
GLboolean GLSLShader::CompileLinkValidate(std::vector<std::pair<GLenum, std::string>> vec) 
{
    ...
    if (GL_FALSE == Link()) { return GL_FALSE; }
    if (GL_FALSE == Validate()) { return GL_FALSE; }
...
{
    if (GL_TRUE == is_linked) { return GL_TRUE; }
    if (pgm_handle <= 0) { return GL_FALSE; }
...
void GLSLShader::Use() 
{
    if (pgm_handle > 0 && is_linked == GL_TRUE) { glUseProgram(pgm_handle); }
}
...
{
    if (pgm_handle <= 0 || is_linked == GL_FALSE) { return GL_FALSE; }
...
GLuint GLSLShader::GetHandle() const { return pgm_handle; }

GLboolean GLSLShader::IsLinked() const { return is_linked; }

std::string GLSLShader::GetLog() const { return log_string; }
...
void GLSLShader::SetUniform(GLchar const *name, GLfloat x, GLfloat y, GLfloat z, GLfloat w) 
{
    GLint loc = glGetUniformLocation(pgm_handle, name);
    if (loc >= 0) 
    {
