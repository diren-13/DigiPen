...
//		textureColorMap.loadFromFile(getAssetPath() + "textures/lena.ktx", VK_FORMAT_R8G8B8A8_UNORM, vulkanDevice, queue, VK_IMAGE_USAGE_SAMPLED_BIT | VK_IMAGE_USAGE_STORAGE_BIT, VK_IMAGE_LAYOUT_GENERAL);
  ...
  void buildComputeCommandBuffer()
  {
    ...
    VkBufferMemoryBarrier memBarrier = vks::initializers::bufferMemoryBarrier();
    memBarrier.buffer               = storagebufferCS.buffer;
    memBarrier.size                 = storagebufferCS.descriptor.range;
    memBarrier.srcAccessMask        = VK_ACCESS_MEMORY_WRITE_BIT;
    memBarrier.dstAccessMask        = VK_ACCESS_SHADER_READ_BIT;
    memBarrier.srcQueueFamilyIndex  = VK_QUEUE_FAMILY_IGNORED;
    memBarrier.dstQueueFamilyIndex  = VK_QUEUE_FAMILY_IGNORED;

    vkCmdFillBuffer(compute.commandBuffer, storagebufferCS.buffer, storagebufferCS.descriptor.offset, storagebufferCS.descriptor.range, 0);
  ...
  void setupDescriptorPool()
  {
    std::vector<VkDescriptorPoolSize> poolSizes = 
    {
      ...
      // Compute pipelines uses a storage buffer for buffer reads and writes
  ...
  void prepareCompute()
  {
    ...
    std::vector<VkWriteDescriptorSet> computeWriteDescriptorSets = 
    {
      ...
      vks::initializers::writeDescriptorSet(compute.descriptorSet, VK_DESCRIPTOR_TYPE_STORAGE_BUFFER, 2,  &storagebufferCS.descriptor)
    };
    ...
    shaderNames = { "histogram", "cdfscan", "apply-Histo"};
  ...
  {
    storagebufferCS.setupDescriptor(sizeof(ssboCS));
  ...
  void updateStorageBuffers()
  {
    ...
    memcpy(storagebufferCS.mapped, &ssboCS, sizeof(ssboCS));
  }
